...
import { ShortLinkStatus } from '../generated/prisma/client.js';
...
import {
  ShareAccessPayload,
  ShareAccessTokenService,
} from './share-access-token.service.js';
...
export class RedirectsService {
  ...
  async resolveInternal(
    ...
    if (snapshot.maxVisits !== null) {
      ...
      return {
        ...
        databaseVisitCountIncremented: true,
    ...
    return {
      ...
      cacheType: snapshot.cacheType,
      originalUrl: snapshot.originalUrl,
      databaseVisitCountIncremented: false,
    };
  }
  ...
  private async resolveLimited(snapshot: RedirectSnapshot) {
    ...
    return this.throwUnavailableReason(snapshot.code);
  }
  ...
  private async authorizeWorkspace(link: RedirectSnapshot, userId?: string) {
    if (!userId) {
  ...
  private async throwUnavailableReason(code: string): Promise<never> {
    ...
    if (link.status !== ShortLinkStatus.ACTIVE) {
      throw new NotFoundException('Short link not found');
    }
