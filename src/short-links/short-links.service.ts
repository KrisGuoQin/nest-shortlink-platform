...
import { ShortLinkBloomService } from '../cache/short-link-bloom.service.js';
  ...
  async create(workspaceId: string, userId: string, dto: CreateShortLinkDto) {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        /*
         * The database is the source of truth. Bloom is an
         * optimization, so a Bloom failure must not turn a successful
         * insert into a failed create response. The Bloom service
         * switches itself to fail-open before rethrowing.
         */
        try {
          await this.bloom.add(link.code);
        } catch (error) {
          const message =
            error instanceof Error ? error.message : String(error);

          this.logger.error(
            `Failed to add short link to Bloom: code=${link.code}, error=${message}`,
          );
        }
